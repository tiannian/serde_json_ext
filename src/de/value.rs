...
impl<'a> Value<'a> {
    /// Creates a new `Value` from a `serde_json::Value` with custom config
    pub fn with_config(value: serde_json::Value, config: &'a Config) -> Self {
        Value { value, config }
    }
}
